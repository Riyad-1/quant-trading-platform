...
class FeatureEngine:
    ...
    def calculate_momentum_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            ...
            (pl.col("close") / pl.col("close").shift(60) - 1).alias("roc_60"),
            (pl.col("close") / pl.col("close").shift(120) - 1).alias("roc_120"),
        ])
    ...
    def calculate_breakout_features(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            pl.col("high").rolling_max(window_size=20).alias("high_20d"),
            pl.col("low").rolling_min(window_size=20).alias("low_20d"),
            pl.col("high").rolling_max(window_size=252).alias("high_52w"),
            pl.col("low").rolling_min(window_size=252).alias("low_52w"),
        ])
        ...
        df = df.with_columns([
            (pl.col("close") >= pl.col("high_20d") * 0.995).alias("is_20d_breakout")
        ])
    ...
    def get_feature_columns(self) -> List[str]:
        ...
        return [
            ...
            "volume_sma_20", "relative_volume", "obv",
            "high_20d", "low_20d", "high_52w", "low_52w",
            "dist_52w_high_pct", "is_20d_breakout"
        ]